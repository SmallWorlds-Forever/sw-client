...
const compile = require('./private/compile');
const FileSystem = require('fs');
const { EOL } = require('os');
// const { ViteRuntime } = require('vite/runtime');
require('@dotenvx/dotenvx').config({ path: Path.join(__dirname, '..', '.env.development') }); // load env.development
...
async function start() {
    ...
    console.log(Chalk.greenBright(`[vite] `) + `Dev server running at: ${Chalk.blueBright(`http://localhost:${rendererPort}`)}`);
    copyStaticFiles();
    startElectron();
